 ...
 * This has been replaced in the C++0X proposal by MixerSFMT.
 *
 * RandonMixer2 is adapted from SFMT19937's init_by_array Mutsuo Saito given in
 * http://www.math.sci.hiroshima-u.ac.jp/~m-mat/MT/SFMT/SFMT-src-1.2.tar.gz and
 * is part of the C++0X proposal; see P. Becker, Working Draft, Standard for
 * Programming Language C++, Oct. 2007, Sec. 26.4.7.1,
 * http://www.open-std.org/jtc1/sc22/wg21/docs/papers/2007/n2461.pdf
 *
 * The adaption to the C++ is by <a href="http://charles.karney.info/">
...
namespace RandomLib {
  ...
  RandomEngine<Algorithm, Mixer>::Check(u64::type v, u32::type e,
    ...
    if (e != Algorithm::version)
      throw std::out_of_range(Name() + ": Algorithm mismatch");
    if (m != Mixer::version)
      throw std::out_of_range(Name() + ": Mixer mismatch");
    u32::type check = 0;
    ...
    if (_stride == 0 || _stride > UNINIT/2)
      throw std::out_of_range(Name() + ": Invalid stride");
  ...
  RandomEngine<Algorithm, Mixer>::RandomEngine(std::istream& is, bool bin)
    ...
    if (t != Check(versionr, versione, versionm))
      throw std::out_of_range(Name() + ": Checksum failure");
  }
...
#define SFMT19937_REVSTEP128(I, J, K, L) {		\
  ...
  template<class RandomType>
  void SFMT19937<RandomType>::Transition(long long count, internal_type statev[])
    throw() {

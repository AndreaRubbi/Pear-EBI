...
#if !defined(RANDOMCANONICAL_HPP)
#define RANDOMCANONICAL_HPP "$Id: RandomCanonical.hpp 6489 2008-11-10 21:37:47Z ckarney $"
...
namespace RandomLib {
  ...
  public:
     ...
     **********************************************************************/
    explicit RandomCanonical(seed_type n) throw(std::bad_alloc);
    /**
  ...
  template<class Generator>
  inline RandomCanonical<Generator>::RandomCanonical(seed_type n)
  ...
  inline std::bitset<nbits> RandomCanonical<Generator>::Bits() throw() {
	...
	b <<= (width > ulbits ? width - ulbits : width);
      if (width > ulbits &&	// x doesn't fit into an unsigned long
	  // But on the first time through the loop the most significant bits
	  // may not be needed.
	  (nbits > ((nbits-1)/width) * width + ulbits ||
	   m < nbits)) {
	// Handle most significant width - ulbits bits.
	b |= (unsigned long)(x >> (width > ulbits ? ulbits : 0));
	b <<= ulbits;
      }

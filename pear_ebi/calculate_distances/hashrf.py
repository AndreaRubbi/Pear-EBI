...
def hashrf(file, n_trees, output_file):
    ...
    cmd = '{path}hashrf {file} {n_trees} -p matrix -o {output} 1> /dev/null 2> ./hashrf_err.txt'.format(path = os.path.join(path, 'HashRF', ''),
        file= file, n_trees= n_trees, output= output_file, verbose = 1).replace('\\', '/')

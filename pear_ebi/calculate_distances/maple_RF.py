...
__credits__ = {"Nicola de Maio": 'EMBL-EBI',
...
import shutil, subprocess, pickle
	...
	for tree in tree_list:
		...
		while index < len(nwString):
			...
			# delimiter:nodes
				...
				if name != "": node.name, name = name, ""
				if distStr != "":
					node.dist = float(distStr) * normalizeInputBLen
					if node.dist < 0.0:
						warnings.warn(f"Warning: negative branch length in the input tree: {str(distStr)} ; converting it to positive.")
						node.dist = abs(node.dist)
					distStr = ""
				else: node.dist = defaultBLen
...
def RobinsonFouldsWithDay1985(t2, t1, rooted=False, minimumBLen=0.000006):
	leafNameDict,nodeTable,leafCount,numBranches,leafDistDict, branchLengthDict, sumBranchLengths = t1
	...
	if visitedLeaves < leafCount:
		print(f"There are leaves in the reference that have not been found in this new tree - leafCount {str(leafCount)} visitedLeaves {str(visitedLeaves)}")
		return None, None, None, None, None, None
...
def calculate_distance_matrix(file, n_trees, output_file):
	...
	global func_pool
	def func_pool(i):
		return subprocess.check_output(['pypy3', f'{current}/RF_pypy.py', str(i)], universal_newlines = True)
	...
	if shutil.which('pypy3') is not None:
		...
		distance_matrix = np.array(list(map(lambda res: eval(res.split('\n')[1].strip()), results)))

...
def bash_command(cmd): 
    if os.name == 'nt':
        try:
            subprocess.run(['C:\\cygwin64\\bin\\bash.exe', '-l', cmd],  stderr=subprocess.DEVNULL, stdout=subprocess.DEVNULL)
        except:
            try:
                subprocess.run(['C:\\cygwin\\bin\\bash.exe', '-l', cmd],  stderr=subprocess.DEVNULL, stdout=subprocess.DEVNULL)
            except:
...
def quartet(file, n_trees, output_file):
    ...
    """
    try: path = os.path.dirname(os.path.abspath(__file__))
    except: path = '.'
    cmd = '{path}/tqDist/bin/all_pairs_quartet_dist {file} {output} 1> /dev/null 2> ./tqdist_err.txt'.format(path = path,
        file= file, output= output_file, verbose = 1)
    
    # Runs command
    bash_command(cmd) 
    bash_command(f"tr -s \" \" < {output_file} | sed 's/\s/_/g' > ./tmp_file && cat ./tmp_file > {output_file} && rm ./tmp_file")
    ...
    except: print("quartet failed! check errors in hashrf_err.txt")
...
def triplet(file, n_trees, output_file):
    ...
    try:
        ...
        distance_matrix.to_csv(output_file)
        return distance_matrix
    except: print("triplet failed! check errors in hashrf_err.txt")

...
try:
    ...
    from .calculate_distances import tqdist
...
class tree_set():
    ...
    def plot_2D(self, method, save=False, name_plot=None, static=False, plot_meta = 'SET-ID', plot_set = None, select = False, same_scale = False):
        if type(plot_set) == type(None): plot_set = self.sets
        if method == 'pca':
            if name_plot == None: name_plot='PCA_2D'
            if type(self.embedding_pca2D) == type(None): self.embed('pca', 2)
            fig = graph.plot_embedding(self.embedding_pca2D, self.metadata, 2, save, name_plot, static, plot_meta, plot_set, select, same_scale)
        
        elif method == 'tsne':
            if name_plot == None: name_plot='TSNE_2D'
            if type(self.embedding_tsne2D) == type(None): self.embed('tsne', 2)
            fig = graph.plot_embedding(self.embedding_tsne2D, self.metadata, 2, save, name_plot, static, plot_meta, plot_set, select, same_scale)
        
        else: raise ValueError("'method' can only be either 'pca' or 'tsne' ")
        
        return fig
    
    def plot_3D(self, method, save=False, name_plot=None, static=False, plot_meta = 'SET-ID', plot_set = None, select = False, same_scale = False):
        if type(plot_set) == type(None): plot_set = self.sets
        if method == 'pca':
            if name_plot == None: name_plot='PCA_3D'
            if type(self.embedding_pca3D) == type(None): self.embed('pca', 3)
            fig = graph.plot_embedding(self.embedding_pca3D, self.metadata, 3, save, name_plot, static, plot_meta, plot_set, select, same_scale)
        
        elif method == 'tsne':
            if name_plot == None: name_plot='TSNE_3D'
            if type(self.embedding_tsne3D) == type(None): self.embed('tsne', 3)
            fig = graph.plot_embedding(self.embedding_tsne3D, self.metadata, 3, save, name_plot, static, plot_meta, plot_set, select, same_scale)
    ...
    def get_subset(self, n_required, method = 'sequence'):
        ...
        with console.status("[bold blue]Extracting subsample...") as status:
            if method == 'syst':
                if shutil.which('pypy3') is not None: 
                    command = ['pypy3', f'{current}/subsample/subsample.py', self.file, str(self.n_trees), str(n_required)]
                    res = subprocess.check_output(command, universal_newlines = True).split('\n')
                    subsample_trees, idxs = eval(res[3]), eval(res[4])
                else:
                    console.log('[bold red]Could not find pypy3 on your sytem PATH - using python3...')                
                    subsample_trees, idxs = subsample.subsample(self.file, self.n_trees, n_required, subp = False)
                ...
                if method == 'random':
                    ...
                    subsample_trees, idxs = list(map(lambda elem: elem[1], selection)), list(map(lambda elem: elem[0], selection))
            ...
            with open(file_sub, 'w') as f:
                for i in subsample_trees: f.write(i)
                f.close()
        ...
        idxs_sorted, order = list(map(lambda x:x[1], sorted_elements)), list(map(lambda x:x[0], sorted_elements))   
        ...
        fig1, fig2 = graph.plot_embedding(comp_sorted, meta_sub, 2), graph.plot_embedding(comp_sorted, meta_sub, 3)   
        return fig1, fig2
...
class set_collection(tree_set):
    ...
    def __init__(self, collection = list(), file = 'Set_collection', 
        ...
        self.distance_matrix = pd.read_csv(distance_matrix, header=None).values if distance_matrix else distance_matrix
        self.embedding_pca2D = None
        self.embedding_tsne2D = None
        self.embedding_pca3D = None
        self.embedding_tsne3D = None

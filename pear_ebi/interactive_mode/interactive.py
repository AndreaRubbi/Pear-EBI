...
def usage():
    ...
    print('[green]⣿⣿⣿⣿⣿⣿⣿⣿⠿⣟⠉⡿⠿⣿⣿⣿⣿⣿⣿⣿[bold white] -- Controls --',
          ...
          '[green]⣿⣿⣿⣿⣿⣿⣿⣿⣿⣷⣤⣿⣿⣿⣿⣿⣿⣿⣿⣿[bold white] 8 --> see list of controls', sep = '\n')
...
def calculate_distances(SET):
    ...
    while True:
        try:method = int(input("Method (1:hashrf - 2:weighted hashrf - 3:day\'s rf - 4:quartet - 5:triplet): "))
        except:continue
        if method in (1,2,3,4,5): break
        else: print('[bold orange]Please select 1, 2 or 3')
...
def embedding(SET):
    while True:
        try:method = int(input("Method (1:PCoA - 2:t-SNE): "))
        except:continue
        if method in (1,2): break
        else: print('[bold orange]Please select either 1 or 2')
        
    while True:
        try:dimensions = int(input("Number of dimensions: "))
        except:continue
        if dimensions in (2,3): break
        else: print('[bold orange]Please select either 2 or 3')
...
def plotting(SET):
    ...
    while True:
        ...
        if show in ('y', 'n', 'Y', 'N', 'Yes', 'No', 'yes', 'no', ''): break
        else: print('[bold orange]Please select valid value')
    ...
    if show in ('y', 'Y', 'Yes', 'yes'): fig.show()
    return 0
...
def add_set():
    ...
    while True:
        ...
        try: 
            ...
            break
        except: print('[bold orange]Cannot find the specified file')
    return filename
...
def get_subset(SET):
    ...
    while True:
        while True:
            ...
            except:print('[bold orange]Please select valid value')
        try:
    ...
    if show in ('y', 'Y', 'Yes', 'yes'): fig2.show()
    return 0
